...
namespace Api.Integration.Test.Usuario
{
    public class QuandoRequisitarUsuario : BaseIntegration
    {
        ...
        public async Task E_Possivel_Realizar_CRUD_Usuario()
        {
            ...
            var stringConten = new StringContent(JsonConvert.SerializeObject(updateUserDto), Encoding.UTF8, "application/json");
            response = await client.PutAsync($"{hostApi}users", stringConten);
            ...
            //Get Id depois de executar Delete
            response = await client.GetAsync($"{hostApi}users/{registroAtualizado.Id}");
